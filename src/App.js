...
const App = () => {
 ...
 const [isLoading,setIsLoading] = useState (true);
  useEffect(()=>{
   ...
   axios.get(`https://api.unsplash.com/photos/?client_id=${ACCESS_KEY}&per_page=30`).then(response=>{
  ...
  const searchImage = (query) =>{
   ...
   const filteredImageList = imageList.filter(image=>{
      image.alt_description = image.alt_description === null ? "React" : 
    ...
    <div>
     ...
     {imageList.length>0?imageList.map((image)=>{
      ...
      style={{padding:"20px", textAlign:"center"}}>
        ...
        />
        <br />
      {image.alt_description ? image.alt_description.substring(0,20)
      :"React"}
        </div>
     );
     })
